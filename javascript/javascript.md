...
* [27. 说一下浏览器垃圾回收机制？](#27-说一下浏览器垃圾回收机制)
* [28. 说一说事件循环 Event loop，宏任务与微任务？](#28-说一说事件循环-Event loop，宏任务与微任务)
* [29. 说一说跨域是什么,如何解决跨域问题？](#29-说一说跨域是什么,如何解决跨域问题)
...
```
1 浏览器的垃圾回收机制主要分为引用计数和标记清除两种算法。
2 引用计数是通过跟踪对象的引用次数来判断是否回收，但有个问题是循环引用，比如两个对象互相引用，即使它们不再被使用，引用计数也不会归零，导致内存泄漏。
3 标记清除的过程分为标记阶段和清除阶段。标记阶段从根对象（比如全局对象、当前执行上下文中的变量）出发，遍历所有可达的对象并标记，然后清除未被标记的对象。这种方法解决了循环引用的问题，但可能会产生内存碎片，于是有些实现还会做内存整理，比如标记整理算法。
```

> [【bilibili】5 分钟彻底搞明白 JS 垃圾回收机制](https://www.bilibili.com/video/BV1fs4y1Y7DT/?spm_id_from=333.337.search-card.all.click&vd_source=59a7d9ad927e21d4f309b9c4fd077245)

#### 28. 说一说事件循环 Event loop，宏任务与微任务？

```
1 浏览器的事件循环：执行 js 代码的时候，遇见同步任务，直接推入调用栈中执行，遇到异步任务，将该任务挂起，等到异步任务有返回之后推入到任务队列中，当调用栈中的所有同步任务全部执行完成，将任务队列中的任务按顺序一个一个的推入并执行，重复执行这一系列的行为。  
2 异步任务又分为宏任务和微任务。
3 宏任务：任务队列中的任务称为宏任务，每个宏任务中都包含了一个微任务队列。 
4 微任务：等宏任务中的主要功能都完成后，渲染引擎不急着去执行下一个宏任务，而是执行当前宏任务中的微任务。
5 宏任务包含：执行 script 标签内部代码、setTimeout/setInterval、ajax 请求、postMessageMessageChannel、setImmediate，I/O（Node.js）。
6 微任务包含：Promise、MutonObserver、Object.observe、process.nextTick（Node.js）。
```

#### 29. 说一说跨域是什么,如何解决跨域问题？
